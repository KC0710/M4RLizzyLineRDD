...
partialDep <- function(dat, eq, gbm_hyperparams, vars, B = 100, n.core = 4, n.trees, seed) {
  if (B == 1) return.mod <- TRUE else return.mod <- FALSE
  
  if (B == 1) {
    pred <- runGbm(dat, eq, gbm_hyperparams, vars,
    ...
    pred <- foreach::foreach(
      i = 1:B, .inorder = FALSE,
      .packages = "gbm", .export = c("runGbm",
                                     "extractPD")
    ) %dopar%
      runGbm(dat, eq, gbm_hyperparams, vars,
