...
merg <- function(aq.dat, meteo.data){
  ...
  aq.dat_m <- aq.dat %>% rename(date = DateTime) %>%
    ...
    select(-Year, -Month, -Day,
           -Classification, -Site_combine,
           -Latitude, -Longitude) %>% 
  
  return(aq.dat_m)
}

all_validated_rr_rdydw <- lapply(all_validated_rr_flat, FUN=merg, meteo_seasonality)

save(all_validated_rr_rdydw, file=here("data", "processed", "all_validated_rr_rdydw.RData"))