...
partialDepTuned <- function(dat, eq, gbm_hyperparams, vars, B = 100, n.core = 4, seed) {
  if (B == 1) return.mod <- TRUE else return.mod <- FALSE
  
  if (B == 1) {
    pred <- runGbmTuned(dat, eq, gbm_hyperparams, vars,
    ...
    pred <- foreach::foreach(
      i = 1:B, .inorder = FALSE,
      .export = c("runGbmTuned", "extractPDTuned")
    ) %dopar%
      runGbmTuned(dat, eq, gbm_hyperparams, vars,
